...
| 分析 | 点「🔍 分析电路」：真值表（可导出 CSV）、SOP 表达式、2~4 变量卡诺图；分析面板可勾选参与穷举的输入，未勾选的开关保持当前电平参与运算 |
...
只挑部分输入穷举（其余开关钉在当前电平）与全选两条路径、SOP 与真值表一致、
2~4 变量卡诺图化简（含跨边界圈）、关卡判定走真实求值、HTTP API 集成。共 90 个用例。
